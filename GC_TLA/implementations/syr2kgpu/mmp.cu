...
/* Thread block dimensions use the grid to tile I and J */
#define DIM_THREAD_BLOCK_X #P3
#define DIM_THREAD_BLOCK_Y #P4
#define TILE_K #P5
__global__ void syr2k_kernel(int ni, int nj, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE *a, DATA_TYPE *b, DATA_TYPE *c)
{
	...
	if ((i < NI) && (j < NI))
	{
        ...
        // Revised to be unrolled
        int block_k;
        for(block_k = 0; block_k < NJ; block_k += TILE_K) {
            int k;
            for(k = 0; k < TILE_K; k++) {
                c[i * NI + j] += alpha * a[i * NJ + block_k + k] * b[j * NJ + block_k + k] +
                                 alpha * b[i * NJ + block_k + k] * a[j * NJ + block_k + k];
            }
        }
	}
}
		...
		DATA_TYPE POLYBENCH_2D(C, NI, NI, ni, ni), DATA_TYPE POLYBENCH_2D(C_outputFromGpu, NI, NI, ni, ni))
{
    ...
    // Dummy kernel to ensure context is created and timing is fair
    dummy_kernel<<<1,1>>>();
...
int main(int argc, char *argv[])
{
	...
	int nj = NJ;
    #ifdef SHOW_SIZE
    printf("Performing syr2k on array sizes NI=%d, NJ=%d\n", NI, NJ);
    #endif
